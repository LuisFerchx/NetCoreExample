...
class Action {
    ...
    showColumn(key) {
        const index = this.hideColumnKeys.indexOf(key);
        if (index !== -1) {
...
class LayoutRender extends MobileLayout {
    ...
    renderHeader(header) {
        ...
        for (const column of this.parent.columns) {
            if (this.isColumnVisible(column)) {
                const index = this.parent.actionModule.columnToggleArray.indexOf(column.keyField);
    ...
    renderCardBasedOnIndex(data, index) {
        ...
        if (cardRow) {
            const td = [].slice.call(cardRow.children).filter((e) => e.getAttribute('data-key').replace(/\s/g, '').split(',').indexOf(key.replace(/\s/g, '')) !== -1)[0];
