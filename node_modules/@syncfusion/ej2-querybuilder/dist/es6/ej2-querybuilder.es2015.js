...
let QueryBuilder = class QueryBuilder extends Component {
    ...
    reset() {
        ...
        const bodeElem = this.element.querySelector('.e-group-body');
        bodeElem.innerHTML = '';
    ...
    addRuleSuccessCallBack(args, trgt, rule, col, act, pId, isRlTmp) {
        ...
        if (!args.cancel) {
            ...
            if (!column || (column && !column.ruleTemplate)) {
                let ddlField;
                let ddlValue = this.isImportRules ? this.GetRootColumnName(rule.field) : rule.field;
                ddlField = {
    ...
    changeRuleTemplate(column, element, rule, type) {
        ...
        if (column.ruleTemplate) {
            ...
            let parentGroup = closest(element, '.e-group-container');
    ...
    addGroupElement(isGroup, target, condition, isBtnClick, not) {
    ...
    addGroupSuccess(args, isGroup, eventTarget, condition, isBtnClick, not) {
        if (!args.cancel && (this.element.querySelectorAll('.e-group-container').length <= this.maxGroupCount)) {
            ...
            if (this.headerTemplate) {
                let args = { requestType: 'header-template-create', ruleID: groupElem.id, condition: condition,
    ...
    headerTemplateFn(groupElem, not, condition) {
        let template;
        let templateID = this.element.id + '_header';
        let args;
        ...
        if (this.headerTemplate) {
            ...
            if (this.isReact) {
                ...
                let templateColl = this.headerFn(args, this, groupElem.id, templateID);
                template = (templateColl[0].nodeType === 3) ? templateColl[1] : templateColl[0];
            }
            else {
    ...
    changeField(args) {
        if (args.isInteracted) {
            let fieldElem = closest(args.element, '.e-rule-filter') || closest(args.element, '.e-rule-sub-filter');
    ...
    changeFilter(flt, dl, grID, rl, tmpRl, dArg) {
        if (flt) {
            let ddlValue = this.isImportRules ? this.GetRootColumnName(dl.value) : dl.value;
    ...
    createSubFields(filterElem, rule, tempRule, ddlArgs) {
        let fieldElem = closest(filterElem, '.e-rule-field');
        let subFieldElem;
        let tempElem;
        ...
        let ruleId = closest(tempElem, '.e-rule-container').id;
        ...
        let height = (this.element.className.indexOf('e-device') > -1) ? '250px' : '200px';
        let subFieldData = Object.keys(this.selectedColumn.columns[0]);
        ...
        if (this.isImportRules) {
            let subField = this.selectedColumn.columns;
    ...
    changeRuleValues(filterElem, rule, tempRule, ddlArgs) {
        ...
        if (filterElem) {
            ...
            if (operatorElem.childElementCount) {
                ...
                let fieldObj = getComponent(filterElem.querySelector('.e-dropdownlist'), 'dropdownlist');
                ...
                let dropDownList = new DropDownList(ddlOperator);
    ...
    renderMultiSelect(rule, parentId, i, selectedValue, values) {
        ...
        let multiSelectObj = new MultiSelect(multiSelectValue);
    ...
    renderStringValue(parentId, rule, operator, idx, ruleValElem) {
        ...
        let selectedValue;
        let isTemplate = (typeof columnData.template === 'string');
        ...
        if ((operator === 'in' || operator === 'notin') && (this.dataColl.length || columnData.values)) {
            ...
            let inputobj = new TextBox(txtBox);
    ...
    renderNumberValue(parentId, rule, operator, idx, ruleValElem, itemData, length) {
        ...
        if ((operator === 'in' || operator === 'notin') && (this.dataColl.length || columnData.values)) {
            ...
            let numeric = new NumericTextBox(numericTxt);
                        ...
                        case 'date':
                            {
                                ...
                                let isTemplate = (typeof itemData.template === 'string');
                                if (itemData.value) {
    ...
    processBoolValues(itemData, rule, parentId, i) {
        ...
        let radiobutton = new RadioButton(radioBtn);
    ...
    renderValues(target, itemData, prevItemData, isRender, rule, tempRule, element) {
        let subFldElem = target.previousElementSibling;
        let filtElem = subFldElem.getElementsByTagName('input')[0];
    ...
    actionBeginSuccessCallBack(args, itemData, ruleID, field, target) {
        if (args.renderTemplate) {
            ...
            if (this.isReact) {
                ...
                let valElemColl = this.columnTemplateFn(args, this, ruleID, templateID);
            ...
            if (this.isReact || this.isAngular) {
                ...
                if (remRule && remRule.rules) {
                    for (let r = 0; r < remRule.rules.length; r++) {
                        ...
                        let isTemplateRendered = ruleElemColl[r].querySelector('.e-template-value');
    ...
    deleteRuleSuccessCallBack(args, target) {
        if (!args.cancel) {
            ...
            let isTemplateRendered = clnruleElem.querySelector('.e-template-value');
            detach(clnruleElem);
            if (column && column.ruleTemplate) {
    ...
    getPredicate(rule) {
        ...
        for (let i = 0, len = ruleColl.length; i < len; i++) {
            ...
            if (keys.indexOf('rules') > -1 && ruleColl[i].rules) {
                ...
                let dateOperColl = ['equal', 'notequal', 'greaterthan', 'greaterthanorequal', 'lessthan', 'lessthanorequal'];
    ...
    datePredicate(ruleColl, value, predicate, condition) {
        ...
        let prevDate = new Date(dummyDate.setDate(dummyDate.getDate() - 2));
    ...
    importRules(rule, parentElem, isReset, not) {
        if (!isReset) {
    ...
    renderGroup(condition, parentElem, not) {
