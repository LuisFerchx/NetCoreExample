...
function calculateSize(maps) {
    ...
    const parentHeight = maps.element.parentElement.clientHeight;
    const parentWidth = maps.element.parentElement.clientWidth;
    const containerElementWidth = stringToNumber(maps.element.style.width, containerWidth);
    const containerElementHeight = stringToNumber(maps.element.style.height, containerWidth);
...
function drawSymbols(shape, imageUrl, location, markerID, shapeCustom, markerCollection, maps) {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const border = { color: borderColor, width: borderWidth };
    const opacity = shapeCustom['opacity'];
    let rectOptions;
    const pathOptions = new PathOption(markerID, fill, borderWidth, borderColor, opacity, dashArray, '');
    if (shape === 'Circle') {
...
function clusterTemplate(currentLayer, markerTemplate, maps, layerIndex, markerCollection, layerElement, check, zoomCheck) {
    ...
    maps.trigger('markerClusterRendering', eventArg, (clusterargs) => {
        ...
        let markerCluster = document.getElementById(maps.element.id + '_LayerIndex_0_markerCluster');
...
let Maps = class Maps extends Component {
     ...
     * @private
     */
    markerSelection(selectionSettings, map, targetElement, 
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    data) {
        const border = {
            color: selectionSettings.border.color,
    ...
    renderArea() {
        ...
        if (width > 0 || (background || this.themeStyle.areaBackgroundColor)) {
            const rect = new RectOption(this.element.id + '_MapAreaBorder', background || this.themeStyle.areaBackgroundColor, this.mapsArea.border, 1, this.mapAreaRect);
        ...
        if (!isNullOrUndefined(templateElements) && templateElements.length > 0 &&
            ...
            for (let i = 0; i < templateElements.length; i++) {
                ...
                if (!isNullOrUndefined(templateGroupEle) && templateGroupEle.childElementCount > 0) {
                    ...
                    const offSetLetValue = this.isTileMap ? 0 : (layerOffset.left < elementOffset.left) ?
                        -(Math.abs(elementOffset.left - layerOffset.left)) : (Math.abs(elementOffset.left - layerOffset.left));
                    const offSetTopValue = this.isTileMap ? 0 : (layerOffset.top < elementOffset.top) ?
    ...
    renderBorder() {
        ...
        if ((width > 0 || (this.background || this.themeStyle.backgroundColor)) && isNullOrUndefined(borderElement)) {
            const borderRect = new RectOption(this.element.id + '_MapBorder', this.background || this.themeStyle.backgroundColor, this.border, 1, new Rect(width / 2, width / 2, this.availableSize.width - width, this.availableSize.height - width));
            ...
            if (targetEle.id.indexOf('_LayerIndex_') !== -1 && !this.isTileMap && (this.mouseDownEvent['x'] === this.mouseClickEvent['x'])
                ...
                if (this.layers[layerIndex].geometryType === 'Normal') {
                    if (targetId.indexOf('_shapeIndex_') > -1) {
                        const location = getMousePosition(e.pageX, e.pageY, e.target.parentElement);
    ...
    getGeoLocation(layerIndex, location) {
        ...
        const pageX = location['layerX'] - container.offsetLeft;
        const pageY = location['layerY'] - container.offsetTop;
    ...
    getTileGeoLocation(location) {
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const latLong = this.pointToLatLong(location['layerX'] + this.mapAreaRect.x - (ele.offsetLeft - container.offsetLeft), location['layerY'] + this.mapAreaRect.y - (ele.offsetTop - container.offsetTop));
...
class Legend {
    ...
    drawLegendItem(page) {
        ...
        if (page >= 0 && page < this.totalPages.length) {
            ...
            for (let i = 0; i < this.totalPages[page]['Collection'].length; i++) {
                ...
                const renderOptions = new PathOption(shapeId, collection['Fill'], strokeWidth, strokeColor, legend.opacity, '');
                ...
                if (i === (this.totalPages[page]['Collection'].length - 1)) {
                    ...
                    if (this.page !== 0) {
                        ...
                        const leftPageOptions = new PathOption(map.element.id + '_Left_Page', '#a6a6a6', 0, '#a6a6a6', 1, '', leftPath);
                        ...
                        const rightPageOptions = new PathOption(map.element.id + '_Right_Page', '#a6a6a6', 0, '#a6a6a6', 1, '', rightPath);
            ...
            for (let j = 0; j < this.maps.selectedLegendElementId.length; j++) {
                ...
                if (!isNullOrUndefined(selectedElement)) {
                    ...
                    if (legendSelectionIndex === -1) {
                        ...
                        this.pushCollection(
                        // eslint-disable-next-line max-len
                        selectedElement, this.maps.legendSelectionCollection, this.maps.legendModule.legendCollection[this.maps.selectedLegendElementId[j]], this.maps.layers[layerIndex].shapeSettings);
                    }
                }
            }
        }
    ...
    renderLegendBorder() {
        ...
        const renderOptions = new RectOption(map.element.id + '_Legend_Border', legend.background, legend.border, 1, this.legendBorderRect, null, null, '', '');
...
class Zoom {
    ...
    panning(direction, xDifference, yDifference, mouseLocation) {
        ...
        if (!map.isTileMap) {
            ...
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            const location = this.maps.getGeoLocation(this.maps.layersCollection.length - 1, mouseLocation);
            panArgs = {
            ...
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            const location = this.maps.getTileGeoLocation(mouseLocation);
            panArgs = {
                cancel: false, name: pan, maps: !map.isBlazor ? map : null,
    ...
    alignToolBar() {
        ...
        element.style.left = x + 'px';
        element.style.top = y + 'px';
        const color = this.maps.zoomSettings.highlightColor;
