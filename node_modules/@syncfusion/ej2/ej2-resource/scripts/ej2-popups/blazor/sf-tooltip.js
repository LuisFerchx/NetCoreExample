import { isNullOrUndefined as NOU, formatUnit, EventHandler, attributes, closest, matches } from '@syncfusion/ej2-base';
import { Touch, Browser, Animation as PopupAnimation, getAttributeOrDefault } from '@syncfusion/ej2-base';
import { addClass, setStyleAttribute, removeClass } from '@syncfusion/ej2-base';
import { Popup } from '../src/popup/popup';
import { calculatePosition } from '../src/common/position';
...
var SfTooltip = (function () {
    ...
    SfTooltip.prototype.wireEvents = function (trigger) {
        ...
        for (var _i = 0, triggerList_1 = triggerList; _i < triggerList_1.length; _i++) {
            ...
            for (var _a = 0, targetList_1 = targetList; _a < targetList_1.length; _a++) {
                ...
                if (opensOn === 'Click') {
                    EventHandler.add(target, Browser.touchStartEvent, this.targetClick, this);
                }
                if (opensOn === 'Hover') {
                    if (Browser.isDevice) {
    ...
    SfTooltip.prototype.addDescribedBy = function (target, id) {
        var describedby = (getAttributeOrDefault(target, 'aria-describedby', null) || '').split(/\s+/);
    ...
    SfTooltip.prototype.removeDescribedBy = function (target) {
        var id = getAttributeOrDefault(target, 'data-tooltip-id', null);
        var describedby = (getAttributeOrDefault(target, 'aria-describedby', null) || '').split(/\s+/);
    ...
    SfTooltip.prototype.targetClick = function (e) {
        var target = this.properties.target ? closest(e.target, this.properties.target) :
            this.element;
        if (!NOU(target)) {
    ...
    SfTooltip.prototype.checkForOpen = function (opensOn, element, e) {
        if (element == null || NOU(e)) {
            return false;
        }
        var target = this.properties.target ? closest(e.target, this.properties.target) : this.element;
        if (target == null) {
            return false;
        }
        var isOpenable = true;
        if (opensOn === 'Hover') {
            ...
            if (element === closest(e.target, '.' + ROOT) &&
                getAttributeOrDefault(target, 'data-tooltip-id', null) === null) {
                isOpenable = true;
            }
            else {
                isOpenable = false;
    ...
    SfTooltip.prototype.targetHover = function (e) {
        ...
        var target = this.properties.target ? closest(e.target, this.properties.target) :
            this.element;
        if (NOU(target) || getAttributeOrDefault(target, 'data-tooltip-id', null) !== null) {
            return;
        }
        this.isMultiTarget = this.getTargetList(this.properties.target).length > 1 ? true : false;
        if (this.properties.target && this.isMultiTarget && e && !NOU(e.target) && !NOU((e.relatedTarget))) {
            var currTarget = closest(e.target, this.properties.target);
            var relatedTarget = closest(e.relatedTarget, this.properties.target);
    ...
    SfTooltip.prototype.showTooltip = function (target, showAnimation, e) {
        ...
        var show = function () {
            _this.isContiniousOpen = !NOU(_this.tooltipEle);
    ...
    SfTooltip.prototype.touchEnd = function (e) {
        if (this.tooltipEle && closest(e.target, '.' + ROOT) === null && !this.properties.isSticky) {
            this.hideTooltip(this.properties.animation.close);
        }
    };
    ...
    SfTooltip.prototype.renderContent = function (target) {
        var title = getAttributeOrDefault(target, 'title', null);
        var dataTitle = getAttributeOrDefault(target, 'data-title', null);
        if (!NOU(title) && target) {
