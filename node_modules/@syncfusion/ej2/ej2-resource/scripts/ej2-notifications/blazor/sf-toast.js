import { closest, removeClass, extend, detach, Touch, Browser, Animation, createElement } from '@syncfusion/ej2-base';
...
var SfToast = (function () {
    ...
    SfToast.prototype.show = function (options) {
        ...
        var target = typeof (this.target) === STRING ? document.querySelector(this.target) :
            document.body;
        options.rootElement.style.zIndex = getZindexPartial(options.rootElement) + '';
        ...
        if (this.extendedTimeout > 0 && this.timeOut > 0) {
            EventHandler.add(options.element, MOUSE_OVER, this.toastHoverAction.bind(this, id));
            EventHandler.add(options.element, MOUSE_LEAVE, this.delayedToastProgress.bind(this, id));
        }
        if (options.showProgressBar) {
            this.progressObj[id].progressEle = options.element.querySelector('.' + PROGRESS);
        }
        EventHandler.add(options.element, KEYDOWN, this.keyDownHandler, this);
    };
    ...
    SfToast.prototype.swipeHandler = function (e) {
        var toastEle = closest(e.originalEvent.target, '.' + TOAST_PRE + ':not(.' + TOAST_CONTAINER + ')');
    ...
    SfToast.prototype.setAnimation = function (toastObj) {
        ...
        new Animation(animate).animate(toastObj.element);
    };
