...
var Selection = (function () {
    function Selection(documentEditor) {
        ...
        this.pasteOptions = function (format) {
            if (format === 'Keep source formatting') {
                _this.owner.editor.applyPasteOptions('KeepSourceFormatting');
            }
            else if (format === 'Match destination formatting') {
    ...
    Selection.prototype.fireRequestNavigate = function (fieldBegin) {
        ...
        this.owner.dotNetRef.invokeMethodAsync("RequestNavigate", eventArgs);
    };
    ...
    Selection.prototype.setHyperlinkContentToToolTip = function (fieldBegin, widget, xPos, isFormField) {
        if (fieldBegin) {
            if (!this.toolTipElement) {
                this.toolTipElement = createElement('div', { className: 'e-de-tooltip e-lib e-popup e-control' });
                this.documentHelper.viewerContainer.appendChild(this.toolTipElement);
            }
            this.toolTipElement.style.display = 'block';
            var toolTipText = this.owner.defaultLocale["Click to follow link"] ? this.owner.defaultLocale["Click to follow link"] : 'Click to follow link';
            ...
            var position = this.getTooltipPosition(fieldBegin, xPos, this.toolTipElement.clientWidth, this.toolTipElement.offsetHeight, false);
            this.showToolTip(position.x, position.y);
            if (isNullOrUndefined(this.toolTipField) ||
                !isNullOrUndefined(this.toolTipField) && fieldBegin !== this.toolTipField) {
    ...
    Selection.prototype.getTooltipPosition = function (fieldBegin, xPos, clientWidth, clientHeight, isFormField) {
        ...
        if ((left + clientWidth + 10) > containerWidth) {
            left = left - ((clientWidth - (containerWidth - left)) + 15);
        }
        var offsetHeight = !isFormField ? clientHeight : 0;
    ...
    Selection.prototype.createPasteElement = function (top, left) {
        if (!this.pasteElement) {
            var htmlElement = this.documentHelper.viewerContainer.getElementsByClassName('e-de-paste')[0];
            this.pasteElement = htmlElement.parentElement.parentElement;
        }
        ...
        this.pasteElement.style.zIndex = '2000';
    };
    ...
    Selection.prototype.onKeyDownInternal = function (event, ctrl, shift, alt) {
        ...
        if (ctrl && !shift && !alt) {
            ...
            switch (key) {
                ...
                case 70:
                    event.preventDefault();
                    this.owner.showDialog("FindPane");
                    break;
            }
        }
    ...
    Selection.prototype.selectComment = function (comment) {
        if (!isNullOrUndefined(comment)) {
            ...
            this.selectPosition(startPosition, endPosition);
            this.documentHelper.currentSelectedComment = comment;
            this.owner.dotNetRef.invokeMethodAsync('SetSelectedComment', comment.commentId);
        }
    };
    Selection.prototype.hightlightCommentSelectionMark = function () {
        if (this.documentHelper.currentSelectedComment) {
            var commentStart = this.getCommentStart(this.documentHelper.currentSelectedComment);
            if (commentStart && !commentStart.commentMark) {
                commentStart.renderCommentMark();
            }
            classList(commentStart.commentMark, ['e-de-cmt-mark-selected'], []);
        }
    };
    Selection.prototype.updatePreviousCommentMark = function (comment) {
        if (this.documentHelper.currentSelectedComment) {
            var commentStart = this.getCommentStart(this.documentHelper.currentSelectedComment);
            var commentMark = commentStart.commentMark;
            if (commentMark) {
                classList(commentMark, [], ['e-de-cmt-mark-selected']);
                this.removeSelectionMark('e-de-cmt-mark-selected');
            }
        }
    };
    Selection.prototype.removeSelectionMark = function (className) {
        if (this.documentHelper.pageContainer) {
            var elements = this.documentHelper.pageContainer.getElementsByClassName(className);
            for (var i = 0; i < elements.length; i++) {
                classList(elements[i], [], [className]);
            }
        }
    };
    Selection.prototype.getCommentStart = function (comment) {
        ...
        if (commentStart.commentMark !== undefined) {
            commentStart.commentMark.title = 'Click to see this comment';
        }
        return this.getFirstCommentInLine(commentStart);
    };
    Selection.prototype.getFirstCommentInLine = function (commentStart) {
