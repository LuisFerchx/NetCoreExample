...
var Editor = (function () {
    ...
    Editor.prototype.deleteCommentWidget = function (comment) {
        ...
        var deleteCommentJson;
        var deleteCommentObj = {};
        deleteCommentObj.comment = this.getCommentObj(comment);
        deleteCommentJson = JSON.stringify(deleteCommentObj);
        this.owner.dotNetRef.invokeMethodAsync('DeleteCommentWidget', deleteCommentJson);
    };
    ...
    Editor.prototype.addCommentWidget = function (commentWidget, isNewComment, showComments, selectComment) {
        if (this.documentHelper.comments.indexOf(commentWidget) === -1) {
            ...
            var newCommentJson = void 0;
            var newCommentObj = {};
            newCommentObj.comment = this.getCommentObj(commentWidget);
            newCommentObj.index = this.documentHelper.comments.indexOf(commentWidget);
            newCommentObj.isNewComment = isNewComment;
            newCommentJson = JSON.stringify(newCommentObj);
            this.owner.dotNetRef.invokeMethodAsync('AddCommentWidget', newCommentJson);
        }
    };
    Editor.prototype.addReplyComment = function (comment, hierarchicalIndex) {
        ...
        var replyCommentJson;
        var replyCommentObj = {};
        replyCommentObj.comment = this.getCommentObj(comment);
        replyCommentObj.index = ownerComment.replyComments.indexOf(comment);
        replyCommentJson = JSON.stringify(replyCommentObj);
        this.owner.dotNetRef.invokeMethodAsync('AddReplyComment', replyCommentJson);
    };
    Editor.prototype.getCommentObj = function (commentEle) {
        var comment = {};
        comment.date = commentEle.date;
        comment.text = commentEle.text;
        comment.isReply = commentEle.isReply;
        comment.isResolved = commentEle.isResolved;
        comment.author = commentEle.author;
        comment.initial = commentEle.initial;
        comment.commentId = commentEle.commentId;
        comment.userColor = this.documentHelper.getAuthorColor(commentEle.author);
        if (commentEle.ownerComment && commentEle.ownerComment.commentId) {
            comment.ownerCommentId = commentEle.ownerComment.commentId;
        }
        return comment;
    };
    ...
    Editor.prototype.stopProtection = function (password) {
        if (this.documentHelper.isDocumentProtected) {
            ...
            unProtectDocumentHandler.url = this.owner.serviceUrl;
            ...
            switch (key) {
                ...
                case 72:
                    ...
                    if (!this.owner.isReadOnly) {
                        this.owner.showDialog("ReplacePane");
                    }
                    break;
                case 73:
                    event.preventDefault();
                ...
                case 75:
                    event.preventDefault();
                    this.owner.showDialog("Hyperlink");
                    break;
                case 76:
                    if (!this.owner.isReadOnlyMode || this.selection.isInlineFormFillMode()) {
    ...
    Editor.prototype.pasteAjax = function (content, type) {
        ...
        this.pasteRequestHandler.url = proxy.owner.serviceUrl;
    ...
    Editor.prototype.paste = function (sfdt, defaultPasteOption) {
        ...
        if (sfdt) {
            ...
            this.applyPasteOptions(this.currentPasteOptions);
            if (this.chartType) {
                setTimeout(function () { _this.viewer.updateScrollBars(); }, 30);
                this.chartType = false;
            }
        }
    };
    ...
    Editor.prototype.handleTableFormat = function (tableFormat, applyFormat) {
        ...
        if (!this.isBordersAndShadingDialog) {
            ...
            if (applyFormat.hasValue('topMargin') && applyFormat.topMargin !== tableFormat.topMargin) {
                tableFormat.topMargin = applyFormat.topMargin;
            }
            if (applyFormat.hasValue('bottomMargin') && applyFormat.bottomMargin !== tableFormat.bottomMargin) {
                tableFormat.bottomMargin = applyFormat.bottomMargin;
            }
            if (applyFormat.hasValue('leftMargin') && applyFormat.leftMargin !== tableFormat.leftMargin) {
                tableFormat.leftMargin = applyFormat.leftMargin;
            }
            if (applyFormat.hasValue('rightMargin') && applyFormat.rightMargin !== tableFormat.rightMargin) {
                tableFormat.rightMargin = applyFormat.rightMargin;
            }
            if (applyFormat.hasValue('cellSpacing') && applyFormat.cellSpacing !== tableFormat.cellSpacing) {
                tableFormat.cellSpacing = applyFormat.cellSpacing;
            }
        }
    ...
    Editor.prototype.handleCellFormat = function (cellFormat, applyFormat) {
        if (!isNullOrUndefined(cellFormat) && !isNullOrUndefined(applyFormat)) {
            if (this.isBordersAndShadingDialog) {
                ...
                if (cellFormat.hasValue('leftMargin') && applyFormat.leftMargin !== cellFormat.leftMargin) {
                    applyFormat.leftMargin = cellFormat.leftMargin;
                }
                if (cellFormat.hasValue('rightMargin') && applyFormat.rightMargin !== cellFormat.rightMargin) {
                    applyFormat.rightMargin = cellFormat.rightMargin;
                }
                if (cellFormat.hasValue('topMargin') && applyFormat.topMargin !== cellFormat.topMargin) {
                    applyFormat.topMargin = cellFormat.topMargin;
                }
                if (cellFormat.hasValue('bottomMargin') && applyFormat.bottomMargin !== cellFormat.bottomMargin) {
                    applyFormat.bottomMargin = cellFormat.bottomMargin;
                }
            }
        }
    };
    ...
    Editor.prototype.onParagraphFormat = function (paragraphFormat) {
        var selection = this.documentHelper.selection;
        var isListBidi = paragraphFormat.bidi && selection.paragraphFormat.listId !== -1;
        if (!isListBidi) {
            this.documentHelper.layout.isBidiReLayout = true;
        }
        this.initHistory('ParagraphFormat');
        this.documentHelper.owner.isShiftingEnabled = true;
        if (this.documentHelper.selection.isEmpty) {
            this.applyParaFormatProperty(selection.start.paragraph, undefined, paragraphFormat, false);
            this.layoutItemBlock(selection.start.paragraph, false);
        }
        else {
            this.updateSelectionParagraphFormatting('ParagraphFormat', paragraphFormat, false);
        }
        this.reLayout(selection);
        if (!isListBidi) {
            this.documentHelper.layout.isBidiReLayout = false;
