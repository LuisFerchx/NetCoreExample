...
import { LineSegment, PointRegion } from '../../smithchart/utils/utils';
...
var SeriesRender = (function () {
    ...
    SeriesRender.prototype.draw = function (smithchart, axisRender, bounds) {
        ...
        for (var m = 0; m < seriesLength; m++) {
            var seriesIndex = m;
            chartAreaRadius = axisRender.areaRadius;
            this.pointsRegion[m] = [];
            this.location[m] = [];
            ...
            chartAreaCx = axisRender.circleCenterX;
            chartAreaCy = axisRender.circleCenterY;
            var diameter = axisRender.areaRadius * 2;
            reactanceStartPoint = { x: chartAreaCx + ((smithchart.renderingType.toLowerCase() === 'impedance') ?
                    chartAreaRadius : -chartAreaRadius), y: chartAreaCy };
            resistantCy = chartAreaCy;
            reactanceCx = reactanceStartPoint.x;
            for (var k = 0; k < series[m].points.length; k++) {
                resistance = this.xValues[k];
                resistantR = (diameter * (1 / (resistance + 1))) / 2;
                reactance = this.yValues[k];
                ...
                interSectPoint = axisRender.intersectingCirclePoints(reactanceCx, reactanceCy, reactanceR, resistantCx, resistantCy, resistantR, smithchart.renderingType);
                var epsilon = _getEpsilonValue();
                if (Math.abs(reactance) < epsilon) {
                    interSectPoint.x = (smithchart.renderingType.toLowerCase() === 'impedance') ?
        ...
        for (var j = 0; j < smithchart.series.length; j++) {
            if (smithchart.series[j].enableSmartLabels) {
                ...
                for (var k = 0; k < smithchart.series[j].points.length; k++) {
                    ...
                    var symbolElement = document.getElementById(smithchart.element.id + '_Series' + j + '_Points' +
                        k + '_dataLabel_symbol' + k);
                    symbolElement.setAttribute('x', currentPoint.xPosition.toString());
                    symbolElement.setAttribute('y', currentPoint.yPosition.toString());
                    var textElement = document.getElementById(smithchart.element.id + '_Series' + j + '_Points' +
                        k + '_dataLabel_displayText' + k);
                    textElement.setAttribute('x', currentPoint.x.toString());
                    textElement.setAttribute('y', currentPoint.y.toString());
