...
var AxisRender = (function () {
    ...
    AxisRender.prototype.renderArea = function (smithchart, bounds) {
        ...
        if (smithchart.radialAxis.axisLine.visible) {
    ...
    AxisRender.prototype.calculateChartArea = function (smithchart, bounds) {
        ...
        this.chartArea = { x: x, y: y, width: chartAreaWidth, height: chartAreaHeight };
    };
    AxisRender.prototype.calculateCircleMargin = function (smithchart, bounds) {
        var padding = 10;
        var maxLabelWidth = 0;
        var width = this.chartArea.width;
        var radius = smithchart.radius;
        maxLabelWidth = this.maximumLabelLength(smithchart);
        var labelMargin = (smithchart.radialAxis.labelPosition.toLowerCase() === 'outside') ? (maxLabelWidth + padding) : padding;
        ...
        this.circleLeftX = this.chartArea.x + labelMargin + (actualRadius * (1 - circleCoefficient));
        this.circleTopY = this.chartArea.y + labelMargin + (actualRadius * (1 - circleCoefficient));
    ...
    AxisRender.prototype.maximumLabelLength = function (smithchart) {
        ...
        var font = smithchart.horizontalAxis.font;
        var label;
        var textSize;
    ...
    AxisRender.prototype.calculateXAxisRange = function (smithchart) {
        ...
        for (var i = 0; i < horizontalAxisLabels.length; i++) {
            ...
            if (smithchart.renderingType.toLowerCase() === 'impedance') {
    ...
    AxisRender.prototype.measureHMajorGridLines = function (smithchart) {
        ...
        for (var i = 0; i < this.horizontalLabelCollections.length; i++) {
            ...
            if (circlePoint.value !== 0.0 && circlePoint.value !== 50.0) {
                startPoint = this.intersectingCirclePoints(radialPoint1[0].centerX, radialPoint1[0].centerY, radialPoint1[0].radius, circlePoint.centerX, circlePoint.centerY, circlePoint.radius, smithchart.renderingType);
                endPoint = this.intersectingCirclePoints(radialPoint2[0].centerX, radialPoint2[0].centerY, radialPoint2[0].radius, circlePoint.centerX, circlePoint.centerY, circlePoint.radius, smithchart.renderingType);
                this.majorHGridArcPoints.push({
                    startPoint: startPoint,
                    endPoint: endPoint,
                    rotationAngle: 2 * Math.PI,
                    sweepDirection: (smithchart.renderingType.toLowerCase() === 'impedance') ?
    ...
    AxisRender.prototype.measureRMajorGridLines = function (smithchart) {
        ...
        for (var i = 0; i < this.radialLabelCollections.length; i++) {
            ...
            if (y !== 0.0) {
                ...
                this.labelCollections.push({
                    centerX: (smithchart.renderingType.toLowerCase() === 'impedance') ?
                        (this.circleCenterX - this.areaRadius) : (this.circleCenterX + this.areaRadius),
                    centerY: this.circleCenterY,
                    angle: (smithchart.renderingType.toLowerCase() === 'impedance') ? 180 : 360,
                    value: y,
                    radius: this.areaRadius,
                    region: null
                });
    ...
    AxisRender.prototype.calculateMajorArcStartEndPoints = function (radialPoint, value, smithchart) {
        ...
        if (value >= 10) {
            arcPoints[0] = (smithchart.renderingType.toLowerCase() === 'impedance') ?
        ...
        if (circlePoint.length > 0) {
            arcPoints[0] = this.intersectingCirclePoints(radialPoint.centerX, radialPoint.centerY, radialPoint.radius, circlePoint[0].centerX, circlePoint[0].centerY, circlePoint[0].radius, smithchart.renderingType);
        }
        arcPoints[1] = this.intersectingCirclePoints(radialPoint.centerX, radialPoint.centerY, radialPoint.radius, cx, cy, this.areaRadius, smithchart.renderingType);
        return arcPoints;
    };
    ...
    AxisRender.prototype.intersectingCirclePoints = function (x1, y1, r1, x2, y2, r2, renderingType) {
        var cx;
        var cy;
        var midRadius;
        var radiusSquare;
        ...
        iy1 = fy + gy;
        iy2 = fy - gy;
        if (renderingType.toLowerCase() === 'impedance') {
            if (ix2 < ix1) {
                point.x = ix2;
                point.y = iy2;
            }
    ...
    AxisRender.prototype.drawHAxisLabels = function (smithchart) {
        ...
        for (var i = 0; i < this.horizontalLabelCollections.length; i++) {
            ...
            if (circleAxis.value !== 0.0) {
                x = (smithchart.renderingType.toLowerCase() === 'impedance') ?
                    circleAxis.centerX - circleAxis.radius : circleAxis.centerX + circleAxis.radius;
                y = circleAxis.centerY;
                textSize = measureText(label, font);
                x = (smithchart.renderingType.toLowerCase() === 'impedance') ? x - textSize.width : x;
                if (hAxis.labelPosition.toLowerCase() === 'outside') {
                    y -= textSize.height / 4;
                }
                else {
                    y += textSize.height;
                ...
                if (hAxis.labelIntersectAction.toLowerCase() === 'hide') {
                    ...
                    for (var j = 1; j < i; j++) {
                        ...
                        if ((smithchart.renderingType.toLowerCase() === 'impedance') && (preX + preWidth) > (curX)) {
                            label = '';
                        }
                        if ((smithchart.renderingType.toLowerCase() === 'admittance') && (preX) < curX + curWidth) {
                            label = '';
                        }
                    }
                }
                var options = new TextOption(smithchart.element.id + '_HLabel_' + i, x, y, 'none', label.toString());
    ...
    AxisRender.prototype.drawRAxisLabels = function (smithchart) {
        var paddingRadius = 2;
        var font = smithchart.radialAxis.font;
        ...
        for (var i = 0; i < this.labelCollections.length; i++) {
            ...
            if (rAxis.labelPosition.toLowerCase() === 'outside') {
            ...
            if (rAxis.labelIntersectAction.toLowerCase() === 'hide') {
            ...
            var options = new TextOption(smithchart.element.id + '_RLabel_' + i, textPosition.x, textPosition.y, 'none', label.toString());
            var element = renderTextElement(options, font, smithchart.themeStyle.axisLabel, groupEle);
            groupEle.appendChild(element);
        }
        smithchart.svgObject.appendChild(groupEle);
    ...
    AxisRender.prototype.measureHMinorGridLines = function (smithchart) {
        ...
        for (var i = 0; i < this.horizontalLabelCollections.length - 3; i++) {
            ...
            for (var j = 0; j < count; j++) {
                ...
                leftX = (smithchart.renderingType.toLowerCase() === 'impedance') ?
                ...
                startPoint = this.intersectingCirclePoints(radialPoint1[0].centerX, radialPoint1[0].centerY, radialPoint1[0].radius, cx, previous['centerY'], radius, smithchart.renderingType);
                endPoint = this.intersectingCirclePoints(radialPoint2[0].centerX, radialPoint2[0].centerY, radialPoint2[0].radius, cx, previous['centerY'], radius, smithchart.renderingType);
                size = { width: radius, height: radius };
    ...
    AxisRender.prototype.measureRMinorGridLines = function (smithchart) {
        ...
        arcStartX = arcCx = (smithchart.renderingType.toLowerCase() === 'impedance') ?
            this.circleCenterX + this.areaRadius : this.circleCenterX - this.areaRadius;
        ...
        for (var i = 2; i < this.labelCollections.length - 3; i++) {
            ...
            for (var j = 1; j < count; j++) {
                ...
                var innerInterSectPoint = this.intersectingCirclePoints(arcCx, arcCy, radius, circlePoint[0].centerX, circlePoint[0].centerY, circlePoint[0].radius, smithchart.renderingType);
    ...
    AxisRender.prototype.arcRadius = function (startPoint, endPoint, angle) {
        ...
        var radius;
        var radian;
        radian = angle > 180 ? (90 * Math.PI / 180) : (270 * Math.PI / 180);
