    ...
    updateParentSelection(parentRecord) {
        ...
        if (!isNullOrUndefined(record)) {
            for (let i = 0; i < childRecords.length; i++) {
                const childRecord = this.parent.getCurrentViewRecords().filter((e) => {
                    return e.uniqueID === childRecords[i].uniqueID;
                });
                const currentRecord = getParentData(this.parent, childRecords[i].uniqueID);
                const checkBoxRecord = (isBlazor() && this.parent.dataSource[adaptorName] === 'BlazorAdaptor') ?
...
class Render {
    ...
    updateTreeCell(args, cellElement) {
        const treeColumn = this.parent.columns[this.parent.treeColumnIndex];
...
class DataManipulation {
    ...
    convertToFlatData(data) {
        ...
        if ((isRemoteData(this.parent) && !isOffline(this.parent)) && data instanceof DataManager && !(data instanceof Array)) {
            ...
            if (this.parent.parentIdMapping) {
                ...
                const clientRender = 'isClientRender';
    ...
    fetchRemoteChildData(rowDetails) {
        ...
        dm.executeQuery(qry).then((e) => {
            const datas = this.parent.grid.currentViewData;
...
let TreeGrid = TreeGrid_1 = class TreeGrid extends Component {
    ...
    render() {
        ...
        if (isBlazor() && this.isServerRendered) {
            const fn = (args) => this.gridRendered(args);
    ...
    extendedGridEditEvents() {
        const keypressed = 'key-pressed';
        const editKeyPress = 'keyPressed';
        const localobserver = 'localObserver';
        let cellEdit$$1;
        let cellSave$$1;
        if (isBlazor() && this.isServerRendered) {
    ...
    bindedDataSource() {
        ...
        const ready = 'ready';
        const adaptorName = 'adaptorName';
        const dotnetInstance = 'dotnetInstance';
        const key = 'key';
        ...
        this.grid.actionComplete = (args) => {
            if (isBlazor() && this.isServerRendered && args.requestType !== 'filterAfterOpen') {
                ...
                for (let i = 0; i < rows.length; i++) {
                    ...
                    const cells = rows[i].querySelectorAll('.e-rowcell');
                    const expandicon = cells[this.treeColumnIndex].getElementsByClassName('e-treegridcollapse')[0] ||
    ...
    onPropertyChanged(newProp) {
        ...
        const preventUpdate = 'preventUpdate';
...
class Edit$1 {
    ...
    blazorTemplates(args) {
        if (isBlazor() && this.parent.isServerRendered) {
            ...
            const colModel = 'columnModel';
            const columnModel = this.parent.grid[colModel];
            const str = 'isStringTemplate';
            for (let i = 0; i < cols.length; i++) {
                if (columnModel[i].template) {
                    const templateID = this.parent.grid.element.id + cols[i].uid;
...
class VirtualTreeContentRenderer extends VirtualContentRenderer {
    ...
    dataBoundEvent() {
        let dataBoundEve = 'dataBound';
        let initialRowTop = 'initialRowTop';
...
class VirtualScroll$1 {
    ...
    constructor(parent) {
        ...
        this.prevendIndex = -1;
        this.parent = parent;
        const injectedModules = 'injectedModules';
        const modules = Grid.prototype[injectedModules];
        for (let i = 0; i < modules.length; i++) {
